...
# Open doors
execute positioned ~-28 ~ ~-28 as @e[type=marker,tag=pk_higuru_temple_marker,tag=pk_inner_door,dx=55,dy=128,dz=55] if score @s PKTrigger = @e[type=marker,tag=pk_boss_room,limit=1] PKTrigger at @s run function pk_strc:higuru_temple/mechanics/inner_door/open
...
# Remove the remaining markers (except ancient altar)
